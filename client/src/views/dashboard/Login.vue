                ...
                <v-col cols="12" md="4">
                    ...
                    >
                        <v-card-text class="text-center">
                            <h6 class="display-1 mb-1 grey--text">Welcome to TSPay Admin panel :)</h6>
                            ...
                            <v-btn color="primary" rounded class="mr-0" @click="validate()">Login</v-btn>
                        </v-card-text>
                    </base-material-card>
                </v-col>
            </v-row>
...
export default {
    ...
    methods: {
        ...
        login() {
            const baseURI = `/admin/v1/login`
            this.$http
                .post(baseURI, this.admin)
                .then(result => {
                    this.$store.commit('SET_ADMIN', result.data)
                    Cookies.set('token', `Bearer ${result.data.data.token}`)
                    this.$router.push('/')
                })
                .catch(error => {
                    if (!error.response) {
