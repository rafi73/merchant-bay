                        ...
                        >
                            <template v-slot:item="row">
                                <tr>
                                    <td
                                        align="center"
                                    >{{ (pagination.page -1 )* pagination.itemsPerPage + (permissions.indexOf(row.item) +1) }}</td>
                ...
                <v-card-text>
                    <template>
                        ...
                        >
                            ...
                            <v-checkbox v-model="permission.is_active" label="Is Active?"></v-checkbox>
                        </v-form>
                    </template>
                </v-card-text>
                <v-card-actions>
...
export default {
    ...
    methods: {
        fetchItems() {
            this.loading = true
            const baseURI = `/admin/v1/permissions?limit=${this.pagination.itemsPerPage}&page=${this.pagination.page}&sort=${this.sort}`
            this.$http
                .get(baseURI)
                .then(result => {
                    this.permissions = result.data.data.rows
        ...
        create() {
            const baseURI = `/admin/v1/permissions`
            this.$http
                .post(baseURI, this.permission)
                .then(result => {
                    this.dialog = false
                    this.fetchItems()
        ...
        update() {
            const baseURI = `/admin/v1/permissions/${this.permission.id}`
            this.$http
                .put(baseURI, this.permission)
                .then(result => {
                    this.dialog = false
                    this.fetchItems()
                    this.showSnackbar(result.data.message, `success`)
